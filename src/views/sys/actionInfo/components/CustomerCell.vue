<template>
  <BasicTable :loading="false" :striped="false" :scroll="{ x: 0, y: 0 }" @register="registerTable">
    ...
    <template #imageUrl="{ text }">
      <TableImg :size="50" :simpleShow="true" :imgList="[text]" />
    </template>
    ...
    <template #toolbar>
      <a-button @click="AddModal">
        <span
          ...
          class="iconify"
          data-icon="fxemoji:bullhorn"
        ></span>
        <span> 添加活动信息 </span>
      </a-button>
    </template>
  </BasicTable>
  <Modal4 @register="register4" @seccecc="handleOG" />
</template>
<script lang="ts">
