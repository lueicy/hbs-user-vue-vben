...
- **Table**
  ...
  - Fix the problem that `ion-change` event is not triggered when `clickToRow` is enabled.
  ...
  - Repair the problem that when rowion.onChange is provided for table, the ed items of table cannot be changed manually.
...
- The `Api` component will automatically re-fetch the data after the `params` is changed
...
- Fixed the problem that the half-ed state of the `Column Display` checkbox of `BaiscTable` was incorrectly displayed
...
- Added icon or component (support local and online methods)
...
- Fix the table cannot get the ed row correctly
...
- `Api` adds `numberToString` property, which is used to convert all the value of `number` into `string`
...
- Added `clickToRow` attribute. Used to control whether the clicked row is checked or not
...
- form: Added remote drop-down `Api` and examples
...
- Delete the four types `OptGroup`, `Option`, `Transfer`, and `Radio` from the form `ComponentType`. Modify the `RadioButtonGroup` component
...
- New menu and top bar color ion color matching
